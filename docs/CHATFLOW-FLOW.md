...
**Важно:** BuildCRM умеет читать несколько форматов тела. Минимум нужно передать:

- текст сообщения (например `text` или `message.text`);
- номер отправителя (например `from` или `phone`).
...
| Отправить ответ обратно в WhatsApp | Блок «Отправить сообщение» в конструкторе, текст = `reply` из ответа вебхука. |

Конструктор в ChatFlow должен выглядеть именно так: **триггер → один вызов нашего вебхука → одна отправка сообщения с текстом из `reply`**. Дополнительных блоков для «логики» или «AI» в самом ChatFlow для этой схемы не нужно — вся логика у нас в BuildCRM.
