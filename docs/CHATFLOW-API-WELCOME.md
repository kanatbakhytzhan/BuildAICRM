...
## Где настроить приветственные сообщения по теме

**CRM → Настройки → Темы лидов → Изменить** (кнопка у нужной темы)

В форме редактирования темы:
- **Название** — название темы
- **Сценарий для AI** — текст приветствия (сценарий для AI-менеджера)
- **Приветственные сообщения по теме:**
  - **Голосовое приветствие (URL .mp3, .ogg)** — `welcomeVoiceUrl`, отправляется через send-audio
  - **Главное фото приветствия (URL)** — `welcomeImageUrl`, отправляется через send-image
  - **Доп. фото (по одному URL на строку)** — `welcomeImageUrls`, массив фото (прайсы и т.д.), каждое через send-image

URL должны быть доступны из интернета (https://...), т.к. ChatFlow загружает их по ссылке.

---

## Как у нас настроено по темам (технически)

- В **модели темы** (TenantTopic) хранятся:
