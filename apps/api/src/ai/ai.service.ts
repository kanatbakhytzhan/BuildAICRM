...
export class AiService {
  ...
  private extractMetadataFromText(
    ...
    // Размеры: дефисы 8-11-3,5; слэш 8/11/3.5; пробелы 8 11 3.5; запятые 8, 11, 3,5; метры 10м 8м 3м; звёздочка 10*12*3; икс 10x12x3
    const dimensionMatch =
      lower.match(/(\d+(?:[.,]\d+)?)\s*(x|х|\*)\s*(\d+(?:[.,]\d+)?)/) ||
      lower.match(/(\d+(?:[.,]\d+)?)\s*на\s*(\d+(?:[.,]\d+)?)/);
    const threeNumbersMatch =
      lower.match(/(\d+(?:[.,]\d+)?)\s*[*xх×]\s*(\d+(?:[.,]\d+)?)\s*[*xх×]\s*(\d+(?:[.,]\d+)?)/) ||
  ...
  /** Приветствие без другого контента: «Здравствуйте», «Сәлеметсіз бе», «Привет» и т.п. */
  private isGreetingOnly(text: string): boolean {
    const t = (text ?? '').trim().toLowerCase();
    if (!t || t.length > 80) return false;
    const greetingRu = /^(здравствуй(те)?|привет|добрый\s+(день|вечер|утро))\.?\!?\s*$/i.test(t);
    const greetingKz = /^(сәлеметсіз\s+бе|сәлем|салам)\.?\!?\s*$/i.test(t) || /[әғқңүұһөі]/.test(t) && t.length < 40;
    return greetingRu || greetingKz;
  }

  /** Текст стартового пакета по языку (для подстановки при приветствии, если модель вернула [IGNORE]). */
  private getStartPackageText(lang: 'ru' | 'kz'): string {
    if (lang === 'kz') {
...
Алдын ала есептеу үшін: Үйдің ұзындығы, Ені, Биіктігі (терезе/есік — қалай болса сонша).

Сайт: https://alusandwich.kz

Біздің мекенжайымыз: Алматы қ., Ырысты көшесі 46/3а`;
    }
    return `Здравствуйте

Цена за 1 м² наших алюминиевых фасадных сэндвич-панелей:
• 3500 тг/м²
• 4000 тг/м²
• 4500 тг/м²
...
Коротко о разнице:
3500 — эконом
4000 — запечённая краска, гарантия 20 лет
4500 — усиленный алюминий

Доставка по Казахстану 📦

Для предварительного расчёта укажите: длину, ширину, высоту здания (окна/двери — по желанию).
...
Если первое сообщение клиента:
— на русском → весь стартовый пакет отправляется на русском
...
➡️ Язык НЕ меняется до конца диалога
➡️ Смешивание языков ЗАПРЕЩЕНО

❌ НЕ ПИШИ в ответе, что голосовое или каталог отправлены. Медиа отправляет система. Только текст стартового пакета.

📦 СТАРТОВЫЙ ПАКЕТ (РУССКИЙ)
...
📦 СТАРТОВЫЙ ПАКЕТ (ҚАЗАҚША) — только один раз, одним потоком с голосом и каталогом:
...
📋 НОВЫЙ ЧАТ / ПОДРОБНЕЕ: первое сообщение или «подробнее» — верни стартовый пакет (RU или KZ по языку). Система отправит голос + каталог одним потоком. Этот текст НИКОГДА не повторяй второй раз.
Приветствие («Здравствуйте», «Сәлеметсіз бе», «Привет») без предыдущего твоего ответа в чате — обязательно верни стартовый пакет по языку приветствия, не [IGNORE].
...
🚨 КАТАЛОГ: «каталог ламината», «прайс» и т.п. — [IGNORE]. Без текста.

⛔ ЗАПРЕЩЕНО писать в ответе: «Қала», «Не қажет», «Қанша м²», «сколько м²». Никогда не задавай эти вопросы.

🌐 ЯЗЫК: Один ответ = один язык. Либо только казахский, либо только русский. Не смешивай. Не добавляй «пожалуйста» к казахскому тексту. Не добавляй русские слова в казахский ответ и наоборот.

⛔ ОДНО И ТО ЖЕ СООБЩЕНИЕ ПОВТОРЯТЬ ЗАПРЕЩЕНО. [IGNORE].
...
🛑 БАКСТРОЙ — верни [IGNORE] (передача менеджеру).`;
  ...
  private formatMetadataForPrompt(metadata: Prisma.JsonValue | null): string {
    ...
    if (m.dimensions != null && typeof m.dimensions === 'object' && !Array.isArray(m.dimensions)) {
      ...
      if (d.length != null && d.width != null && d.height != null) {
        ...
        const windowsCount = typeof m.windowsCount === 'number' ? m.windowsCount : 0;
        const doorsCount = typeof m.doorsCount === 'number' ? m.doorsCount : 0;
        const subtractArea = windowsCount * 2 + doorsCount * 2;
  ...
  private async generateOpenAIReply(params: {
    ...
    // Убираем из ответа модели фразы про отправку медиа (голосовое/каталог отправляет система отдельно)
    content = content
      .replace(/\[\s*Голосовое сообщение отправлено\s*\]/gi, '')
      .replace(/\[\s*Каталог фото отправлен\s*\]/gi, '')
      .replace(/\[\s*Каталог отправлен\s*\]/gi, '')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  ...
  async handleFakeIncoming(params: { tenantId: string; leadId: string; text: string; skipSaveIncoming?: boolean }) {
    ...
    // Запрещённый блок: никогда не отправлять вопросы «Қала», «Не қажет», «Қанша м²», «Байланыс нөміріңіз»
    if (reply && /(—\s*)?Қала\s*[:\s]|Не\s+қажет|Қанша\s*м²|Байланыс\s+нөміріңіз/i.test(reply)) {
      reply = '[IGNORE]';
    }
    // [IGNORE] / [SILENT] / [HUMAN_TAKEOVER] от модели — ничего не отправляем, кроме приветствия без ответа
    const replyTrim = reply?.trim() ?? '';
    if (/^\[IGNORE\]$/i.test(replyTrim) || /^\[SILENT\]$/i.test(replyTrim) || /^\[HUMAN_TAKEOVER\]$/i.test(replyTrim)) {
      if (this.isGreetingOnly(text) && !alreadySentWelcome) {
        reply = this.getStartPackageText(/[әғқңүұһөі]/.test(text) ? 'kz' : 'ru');
      } else {
        await this.logs.log({
          tenantId,
          category: 'ai',
