...
🟦 ПАНЕЛИ — ГЛАВНОЕ: ДЛИНА, ШИРИНА, ВЫСОТА. Город и окна/двери не обязательны, но если клиент дал — учитывай в расчёте. Размеры в любом формате: 10x12x3, 8-11-3,5, 10м 8м 3м. Формула: периметр = (длина+ширина)×2, площадь стен = периметр×высота. Минус окна/двери если даны. Материал = площадь × 3500 ₸, монтаж = площадь × 4000 ₸. Напиши кратко расчёт. После расчёта — [IGNORE]. Больше ничего не пиши.
...
🛑 БАКСТРОЙ — верни [IGNORE] (передача менеджеру).`;
  ...
  private formatMetadataForPrompt(metadata: Prisma.JsonValue | null): string {
    ...
    if (m.dimensions != null && typeof m.dimensions === 'object' && !Array.isArray(m.dimensions)) {
      ...
      // Площадь СТЕН = периметр × высота. НЕ длина×ширина!
      if (d.length != null && d.width != null && d.height != null) {
        const perimeter = (d.length + d.width) * 2;
        const wallArea = perimeter * d.height;
        parts.push(`Площадь стен для расчёта = (${d.length}+${d.width})×2×${d.height} = ${wallArea} м² (используй это число, минус окна/двери)`);
      }
    }
