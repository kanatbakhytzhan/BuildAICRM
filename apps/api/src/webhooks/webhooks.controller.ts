...
export class WebhooksController {
  ...
  private async handleChatflow(
    ...
    // С таргета/рекламы ChatFlow часто не передаёт welcome=1. «Подробнее» или первое сообщение — сразу стартовый пакет.
    const incomingCount = await this.prisma.message.count({
      where: { leadId: lead.id, direction: MessageDirection.in },
    });
    const isRequestMoreInfo = this.messages.isRequestForMoreInfo(bodyToSave);
    const isFirstIncoming = incomingCount <= 1;
    const treatAsWelcome = isWelcome || isFirstIncoming || isRequestMoreInfo;
    await this.logs.log({
      tenantId,
      category: 'whatsapp',
